...
> **2026-09 업데이트 — 1~3순위는 이미 코드로 만들어져 있습니다.**
>
> | 항목 | 상태 | 파일 |
> |---|---|---|
> | 동물 캐릭터 10종 | ✅ 완료 (SVG) | `src/components/ui/AnimalFace.jsx` |
> | 도시 랜드마크 15종 | ✅ 완료 (SVG) | `src/components/board/LandmarkIcon.jsx` |
> | 특수칸 아이콘 6종 | ✅ 완료 (SVG) | `src/components/board/SpecialIcon.jsx` |
> | 건물 4종 | ✅ 완료 (SVG) | `src/components/board/BuildingIcon.jsx` |
> | 로고 | 코드 버전 있음 | `src/components/ui/Logo.jsx` |
> | 카드·화폐 | 미제작 (선택) | — |
>
> 이미지 파일 대신 **코드에서 직접 그리는 SVG**로 만들었습니다. 이렇게 하면
> 10마리·15개 랜드마크의 선 굵기와 색이 자동으로 똑같아지고, 28px로 줄여도
> 흐려지지 않고, 앱 용량도 거의 늘지 않습니다. 색이나 모양을 바꾸고 싶으면
> 위 파일 맨 위의 색상표(`SPECS`, `LANDMARKS`)만 고치면 됩니다.
>
> 아래 규격서는 **더 멋진 그림을 외주·AI로 따로 받아 교체하고 싶을 때** 쓰세요.
